...
func NewAgent(r *errgroup.Group, cfg *config.Config) *Agent {
	c := resty.New()
...
func (a *Agent) Run(ctx context.Context) {
	...
	a.runner.Go(func() error {
		for {
			...
			case <-ctx.Done():
				return ctx.Err()
			case <-time.After(time.Second * time.Duration(a.cfg.Agent.PollInterval)):
	...
	a.runner.Go(func() error {
		for {
			...
			case <-ctx.Done():
				return ctx.Err()
			case <-time.After(time.Second * time.Duration(a.cfg.Agent.ReportInterval)):
